...
create trigger update_tri before update on メールテーブル for each row execute procedure set_update_time();
...
create table ユーザーマスタ (ユーザーid varchar(20) primary key, 受信者ユーザーid varchar(20), 名前 text, パスワード text, 権限id numeric, 削除フラグ numeric, 更新日時 timestamp not null default current_timestamp, foreign key (権限id) references 権限マスタ (権限id));

create trigger update_tri before update on ユーザーマスタ for each row execute procedure set_update_time();
...
create trigger update_tri before update on 予定テーブル for each row execute procedure set_update_time();


create table 施設テーブル (施設id smallserial primary key,ユーザーid varchar(20), 施設名 text, 開放開始時間 timestamp, 開放終了時間 timestamp, 更新日時 timestamp not null default current_timestamp);

create trigger update_tri before update on 施設テーブル for each row execute procedure set_update_time();
...
create trigger update_tri before update on 施設利用状況テーブル for each row execute procedure set_update_time();



insert into ユーザーマスタ values ('aiueo', '田中太郎', 'pass', 1, 0); 
